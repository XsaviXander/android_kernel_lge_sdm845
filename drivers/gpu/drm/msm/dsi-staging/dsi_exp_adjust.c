...
static int ea_panel_send_pcc(u32 bl_lvl)
{
	...
	if (!crtc) {
		pr_err("Cannot find display panel with CRTC.\n");
		return -ENODEV;
	}

	priv = crtc->dev->dev_private;
	prop = priv->cp_property[1];

	if (prop == NULL) {
		pr_err("PCC is not supported.\n");
		return -EINVAL;
	}

	rc = sde_cp_crtc_get_property(crtc, prop, &val);
	if (rc) {
		pr_err("Cannot get the CRTC property. Things may go wrong.\n");
	}

	pr_debug("The backlight value is %d.\n", bl_lvl);

	if (bl_lvl < ELVSS_OFF_THRESHOLD) {
		ea_coeff =
			bl_lvl * PCC_BACKLIGHT_SCALE + EXPOSURE_ADJUSTMENT_MIN;
	} else {
		ea_coeff = EXPOSURE_ADJUSTMENT_MAX;
	}

	pr_debug("The backlight coefficient is %d.\n", ea_coeff);
	...
	if (IS_ERR_OR_NULL(blob)) {
		pr_err("Failed to create the DSPP blob. Bailing out.\n");
		return -EINVAL;
	}
	...
	if (rc) {
		pr_err("Cannot set PCC property.\n");
	}
...
void ea_panel_mode_ctrl(struct dsi_panel *panel, bool enable)
{
	if (pcc_backlight_enable != enable) {
		...
		pr_debug("Recovering the backlight level (%d).\n", last_level);
...
u32 ea_panel_calc_backlight(u32 bl_lvl)
{
	...
	if (pcc_backlight_enable && bl_lvl != 0 &&
	    bl_lvl < ELVSS_OFF_THRESHOLD) {
		if (ea_panel_send_pcc(bl_lvl))
			pr_err("Failed to send PCC.\n");
